...
const Gradient: FC<{ id: string }> = ({ id }) => {
  ...
  const mixing = CSS.supports('(background: color-mix(in oklch, black, white))');
  const nStops = !mixing ? 0 : 10 + rN(10)
  ...
  const makeStop = (o: number, i: number) => (
    <Fragment key={`${i}-${o}`}>
      <stop offset={r3(o)} stopColor={color(rN(100))} />
      <stop offset={r3(o)} stopColor={grX()} />
    </Fragment>
  )
